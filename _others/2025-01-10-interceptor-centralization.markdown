...
- 팀 프로젝트를 진행하면서 API Call 함수가 여러 개 만들어졌다.  
  아래 그림은 일부만 보여준 것이고, **각 폴더마다 대략 10개의 API Call 함수**가 있다고 보면 된다.
...
- 그래서 프로젝트 회고를 진행하면서, **`instance`의 interceptor에서 공통 처리를 하면 좋겠다**는 생각이 들었고,  
  찾아보니 이를 **axios interceptor 중앙화**라고 부른다는 것을 알게 되었다.
...
- 각 API 함수에서는 **요청과 응답 데이터 처리**에만 집중하고,  
...
- 특히 지금 **React Query 강의**를 수강 중인데, 여기서 **React Query 레벨에서 중앙화**하는 방법을 알게 되었다.  

...
코딩테스트를 **JavaScript**로 준비했다.  
문제를 풀다 보니 **for, while**만 쓰면 코드가 길어지고, **시간 복잡도** 생각도 헷갈렸다.  
**배열·문자열 메서드**와 자주 쓰는 **Set/Map**을 집중적으로 정리하게 됐다.  
...
- **this**가 상황에 따라 달라져서, 함수 안에서 의도와 다르게 동작하는 문제가 발생함
...
- **map, filter, reduce**와 **Set/Map**을 쓰면 반복문을 **한 줄 메서드**로 줄일 수 있다.

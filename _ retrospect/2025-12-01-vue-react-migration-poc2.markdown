...
Next.js가 Facade 역할을 해서, 클라이언트에게 단일 진입점을 제공한다.
...
- 진입점이 Nginx에서 Next.js로 변경
- 라우팅 로직이 nginx.conf에서 middleware.ts로 이동
- 프론트엔드가 제어권 회복
...
판단: 점진적 전환 단계에서는 이 리스크를 감수한다. 라우팅 제어권이 더 중요하고, 전환 완료 후에는 New 앱만 남으므로 해소된다.
...
우리 팀은 Facade 방식을 선택했다. 점진적 전환 단계에서는 속도와 유연성이 더 중요했기 때문이다.

...
컴포넌트를 작성할 때 상태 관리를 위해 useState를 기본으로 썼다.  
간단한 값 하나만 다룰 때는 충분했지만, 상태 개수가 늘어나면서 코드가 점점 복잡해졌다.

특히 여러 상태가 서로 연관되어 있을 때는, useState만으로는 가독성이 떨어지고 유지보수가 어려웠다.  
...
처음에는 상태를 useState로만 관리했다.  
프로젝트가 커지고 관리할 상태가 많아지면서, 같은 컴포넌트 안에 useState가 많이 나열되고 가독성이 급격히 떨어졌다.
...
1. useState처럼 상태를 관리하지만, reducer라는 함수를 통해 상태 업데이트 로직을 한곳에 모아 관리한다.  
   상태와 액션을 분리해서 다룬다.[^1]
...
상태 업데이트 로직을 reducer 한곳에 모아 두었기 때문에, 상태가 늘어나도 state와 dispatch만 신경 쓰면 됐다.  
상태 변경 흐름을 한눈에 파악하기 쉬워졌다.

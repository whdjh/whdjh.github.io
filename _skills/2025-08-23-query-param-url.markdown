...
마이페이지 **탭 상태**를 **Zustand**로만 관리하고 있었다.  
그러다 보니 **페이지 새로고침** 시 상태가 초기화되고, **뒤로가기/앞으로가기**가 제대로 동작하지 않는 **UX 문제**가 생겼다.
...
→ **쿼리 파라미터 기반**으로 탭 상태를 URL에 반영하기로 했다.
...
- **URLSearchParams**로 기존 쿼리를 유지한 채 해당 키만 바꾸고, **router.push**로 URL을 갱신해 **브라우저 히스토리**에 스택으로 쌓이게 한다.

---

## 결과

- **새로고침**해도 탭 상태가 URL에서 복원된다.
